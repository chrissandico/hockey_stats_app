...
void main() async {
  ...
  // In a real app, this would come from Google Sheets initially (UF-6)
  ...
  if (playersBox.isEmpty) { // Only add if the box is empty
     ...
     playersBox.add(Player(id: 'player_5', jerseyNumber: 55, teamId: 'opponent_team')); // Added another opponent for testing
  }
  ...
  var gamesBox = Hive.box<Game>('games');
   if (gamesBox.isEmpty) { // Only add if the box is empty
     gamesBox.add(Game(id: dummyGameId, date: DateTime.now().subtract(Duration(days: 1)), opponent: 'Rivals')); // Use the first dummyGameId, yesterday
     gamesBox.add(Game(id: dummyGameId2, date: DateTime.now(), opponent: 'Chiefs')); // Use the second dummyGameId, today
   }
